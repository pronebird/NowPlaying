...
struct CNotifyIconData : public NOTIFYICONDATA
{
public:	
	CNotifyIconData()
	{
...
public:	
	...
	// 	lpszToolTip 	- The tooltip to display
	//	hIcon 		- The icon to display
	// 	nID		- Application-defined identifier of 
	// 	hMenu		- The context menu handle of the taskbar icon
	/// returns true on success
	bool InstallIcon ( LPCTSTR lpszToolTip, HICON hIcon, UINT nID, CMenu hMenu )
	{
		T* pT = static_cast<T*>(this);
		// Fill in the data		
		m_nid.hWnd = pT->m_hWnd;
	...
	LRESULT OnTrayIcon(UINT /*uMsg*/, WPARAM wParam, LPARAM lParam, BOOL& /*bHandled*/)
	{
		...
		if (LOWORD(lParam) == WM_RBUTTONUP)
		{
			...
			if (m_nDefault == 0)
				m_nid.hMenu.SetMenuDefaultItem(0, TRUE);
			else
				m_nid.hMenu.SetMenuDefaultItem(m_nDefault);
			// Track
			m_nid.hMenu.TrackPopupMenu(TPM_LEFTALIGN, pos.x, pos.y, pT->m_hWnd);
